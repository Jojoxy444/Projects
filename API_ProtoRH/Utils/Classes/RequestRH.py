...
class RequestRH (Base):
    ...
    last_action = Column(Date, index=True)
    delete_date = Column(Date, index=True)
...
class RequestRHContent(BaseModel):
    author: int
    content: str
    date: str
...
class RequestRHGet(BaseModel):
    id: int
    id_user: int
    content: List[RequestRHContent]
    registration_date: date
    visibility: bool
    close: bool
    last_action: date
    delete_date: Optional[date]

class RequestRHGetResponse(BaseModel):
    requests: List[RequestRHGet]
